...
from blusky.utils.pad_2d import Pad2D, pad_to_log2, ReflectionPadding2D
...
class ApplyFatherWavlet2D(HasStrictTraits):
    ...
    def _get__tile_size(self):
        ...
        if size > self.img_size[0] or size > self.img_size[1]:
            ...
            msg = "For image {} by {}, max J is {}".format(self.img_size[0],
                                                           self.img_size[1],
                                                           np.log2(mn) -2)
    ...
    def _convolve(self, input_layer, trainable=False):
        ...
        # power of "2"
        wav = self.wavelet.kernel(0., shape=(2**(self.J+2), 2**(self.J+2)))

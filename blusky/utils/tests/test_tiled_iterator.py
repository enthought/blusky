...
class TestTiledIterator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_file_1 = path.join(path.dirname(datasets.__file__), "twod_image_1.npy")
        self.test_file_2 = path.join(path.dirname(datasets.__file__), "twod_image_2.npy")
    ...
    def test_tiled_iterator_nogen(self):
        ...
        tile_no_gen = TiledIterator(twod_image=self.test_file_1, 
                                    overlap_log_2=0)
    ...
    def test_tiled_data_generator(self):
        ...
        tile_gen = TiledIterator(twod_image=self.test_file_1, 
                                  overlap_log_2=1,
                                  image_data_generator=datagen)

        
        next(tile_gen); next(tile_gen); tile = next(tile_gen)

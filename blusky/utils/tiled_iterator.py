...
def patches(test_data, 
    ...
    tile_iter = TiledIterator(twod_image=test_data, 
                              tile_size=tile_size,
...
class TiledIterator(HasTraits, Iterator):
    """ Generate tiles from a 2d numpy array. Integrates with Keras image generators
        augmentation is needed.
    """
    
    #: (M) This is defines the overlap of the tiles, so overlap_log_2 = 0 would be no
    # overlap,
    #  overlap_log_2 = 1 would be 50% overlap, overlap_log_2 = 2 would be 75% etc.
    overlap_log_2 = Int(0)
    ...
    #: 2-d label mask, if provided a label is assigned from the center of the tiles.
    _label_mask = Array
    
    #: Interpreted as "not labeled", if label mask is provided, tiles associated with
    #  this label will be ignored
    null_label = Int(0)
    
    # Instance of `ImageDataGenerator` to use for random transformations and normalization.
    # If undefined then no augmentation is done, it just creates tiles.
    ...
    def __init__(self, **traits):
        ...
        self.transform_log_scale = int(round(np.log2(np.min(self.tile_size))) - 2)
        ...
        if self.image_padding < 1:
            ...
            self._twod_image = np.pad(self._twod_image, 
                                      (self.image_padding, self.image_padding), 
                                     'reflect')
            if self._label_mask.size > 0:
                self._label_mask = np.pad(self._label_mask, 
                                          (self.image_padding, self.image_padding), 
                                         'reflect')
    ...
    def _get_tiles(self, index_array):
        ...
        x = np.array([self._twod_image[ix-nh:ix+nh+1,iy-nw:iy+nw+1] 
    ...
    def _get_label(self, index_array):
        ...
        # symmetric about centroid, if tile size is even, adds 1 to the shape
        x = np.array([self._label_mask[ix,iy] for ix,iy in self._centroids[index_array]])
    ...
    def fit(self, data_to_standardize):
        ...
        if self.featurewise_center:
            ...
            for i in data_to_standardize:
                ...
                mn += np.array([np.sum(val[...,j]) for j in range(val.shape[2])])
        ...
        if self.featurewise_std_normalization:
            ...
            for i in data_to_standardize:
                ...
                var += np.array([np.var(val[...,j] - mn) for j in range(val.shape[2])])

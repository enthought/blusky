...
def split_angle_range(angles, angle_range, centered=False):
    ...
    if centered:
        ...
        # roll the angle to the center
        center = np.mean(angle_range)
        #indx = (len(_angles) - np.argmin(np.abs(center - _angles)))//2
        indx = len(_angles)//2
...
class Visualize2D(HasStrictTraits):
    ...
    def __init__(self, **traits):
        ...
        self.theta, self.radii = np.meshgrid(np.linspace(ang_min, ang_min+360,
                                               self._num_ang_cell),
    ...
    def _render_element(self, pixel, z, do_log=False):
        ...
        indx = np.logical_and( np.logical_and(self.radii >= radius_range[0],
                                              self.radii <= radius_range[1]), 
                               np.logical_and(self.theta >= angle_range[0], 
                                              self.theta <= angle_range[1])
                         )
        ...
        indx = np.logical_and( np.logical_and(self.radii >= radius_range[0],
                                              self.radii <= radius_range[1]), 
                               np.logical_and(self.theta >= _angle[0], 
                                              self.theta <= _angle[1])
                        )
    ...
    def plot(self, coeffs, ax1, order, do_log=False, cmap='cividis'):
        ...
        [self._render_element(pixel, z, do_log=do_log)
             for pixel in zip(self.plot_elements, coeffs) if pixel[0].order == order];
        ...
        im = ax1.pcolormesh(np.deg2rad(self.theta),
                            self.radii, z,
                            cmap=cmap, norm=norm)
